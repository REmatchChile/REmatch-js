...
class Regex {
  ...
  find(string) {
    let it = this.rgx_object.findIter(string, this.rgxOptions.kUnanchored);
  ...
  *findIter(string) {
    let iter;
    let match;
    iter = this.rgx_object.findIter(string, rematch.kUnanchored);
    while (iter.hasNext()) {
  ...
  match(string) {
    ...
    let it = this.rgx_object.findIter(string, this.rgxOptions.kSingleAnchor);
      if (it.hasNext()){
          m = it.next()
  ...
  fullmatch(string) {
    let it = this.rgx_object.findIter(string, rgxOptions.kBothAnchors);
      if (it.hasNext()){
          m = it.next()
          return new Match(m, string)
      }
  ...
  findall(string) {
    let vector = [];
    let match;
    let it = this.rgx_object.findIter(string, rgxOptions.kUnanchored);
    while (it.hasNext()) {
